			...
			for(var i=0;i<l;i++)
				{
                    ...
                    console.log(postfix)
					var a = $("<div class='mouseovera' id = 'org"+data[i].id+"' style='float:left; width:95%;height:180px;border:1px solid grey;'><div style=width:1%;height:99%;float:left;margin: 0.1%></div><div style='width:90%;height:100%;float:left;margin:2%'><p>认证机构用户名:<b>"+data[i].name+"</b><button style=\"float:right\" type=\"button;\" class=\"btn btn-default\" onclick=\"reject("+data[i].id+")\">拒绝</button><button style=\"float:right\" type=\"button;\" class=\"btn btn-default\" onclick=\"pass("+data[i].id+")\">通过</button></p><p>认证机构负责人真实姓名:<b>"+data[i].realname+"</b></p><p>认证机构机构名:<b>"+data[i].organization+"</b></p><p>认证机构邮箱:<b>"+data[i].email+"</b></p><p><button style=\"float:right\" type=\"button;\" class=\"btn btn-default\" onclick=\"download('"+filename+"','"+postfix+"')\">下载认证资料</button></p></div></div>")
